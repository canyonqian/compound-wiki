...
def _resolve_paths() -> Dict[str, Path]:
    ...
    project_dir = Path(os.environ.get("CAM_PROJECT_DIR", str(PROJECT_ROOT))).resolve()
        ...
        if "---" in existing and "---" in content:
            ...
            for nb in new_blocks:
                ...
                for eb in existing_blocks:
                    ...
                    if ratio >= 0.85:
                        logger.debug(f"  _save_page: skipping duplicate block (ratio={ratio:.2f})")
                        is_dup = True
                        break
            ...
            if len(deduped_new_blocks) < len(new_blocks):
                logger.info(f"  📄 MERGE: {filename} — deduplicated {len(new_blocks) - len(deduped_new_blocks)} blocks")
...
def _search_wiki(
    ...
    for dir_name, dir_path in search_dirs:
        ...
        for md_file in sorted(dir_path.rglob("*.md"), key=lambda p: p.stat().st_mtime, reverse=True):
...
def _get_stats(paths: Dict[str, Path] = None) -> Dict:
    ...
    stats["total"] = stats["concepts"] + stats["entities"] + stats["synthesis"]
    ...
    issues = 0
    if stats["avg_links"] < 2:
        issues += 1
    if stats["concepts"] == 0 and stats["raw_files"] > 0:
        issues += 1
    if stats["total_chars"] < 1000 and stats["total"] > 0:
        issues += 1
...
def _build_extraction_prompt(
    raw_filename: str, content: str, title: str, rules_text: str, paths: Dict[str, Path]
) -> str:
    """
    ...
    if wiki_dir.exists():
        ...
        existing_pages = [p for p in existing_pages if p.name not in ("index.md", "changelog.md")]
    ...
    try:
        ...
        if not content or len(content.strip()) < 10:
            return [
                TextContent(
                    type="text",
                    text="❌ Content too short (need at least 10 characters). "
    ...
    try:
        ...
        for page_data in pages:
            ...
            if result["action"] == "created":
                ...
                updated_count += 1
            results.append(f"✅ {result['action'].upper()}: {result['path']} ({result['size_bytes']} chars)")
