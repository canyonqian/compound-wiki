...
logger = logging.getLogger("cam.pipeline")
...
class LLMClient:
    ...
    def __init__(self, config=None):
        ...
        self.api_key = self.config.get("api_key", "") or os.environ.get("CAM_LLM_API_KEY", "")
        self.base_url = self.config.get("base_url", "") or os.environ.get("CAM_LLM_BASE_URL", "")
...
- Cite sources for every claim"""
    ...
    def _build_ingest_prompt(self, filename: str, content: str) -> list[dict]:
        ...
        system_prompt = f"""You are CAM's Knowledge Enginee. Your role is to read raw materials and convert them into structured, linked Markdown wiki pages.
...
Please analyze this content and generate wiki pages according to your rules."""
    ...
    def _build_query_prompt(self, question: str, context: str) -> list[dict]:
        return [
            {"role": "system", "content": f"""You are CAM's Query Engine. Answer questions based ONLY on the provided wiki content.
