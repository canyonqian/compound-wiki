...
class WikiTransaction:
    ...
    def create_page(self, rel_path: str, content: str, frontmatter: Dict = None) -> None:
    ...
    def update_page(self, rel_path: str, content: str, append_section: str = None) -> None:
    ...
    def _format_fact_as_md(fact) -> str:
        ...
        confidence_bar = "█" * int(fact.confidence * 10) + "░" * (10 - int(fact.confidence * 10))
        ...
        lines = [
            ...
            fact.content,
            "",
            f"*Source*: > {fact.source_text[:150]}..."
            if len(fact.source_text or "") > 150
            else f"*Source*: > {fact.source_text or 'N/A'}",
            "",
        ]
        ...
        if fact.entities_mentioned:
            entities = " · ".join(f"[[{e}]]" for e in fact.entities_mentioned)
...
class SharedWiki:
    ...
    def __init__(self, wiki_path: str, raw_path: str = None, config=None):
        ...
        self.raw_path = Path(raw_path).resolve() if raw_path else self.wiki_path.parent / "raw"
    ...
    async def transaction(self, agent_id: str = "default") -> WikiTransaction:
        ...
        # Acquire lock
        lock = FileLock(self._lock_path, timeout=self.config.concurrency.lock_timeout_seconds)

        acquired = lock.acquire()
        if not acquired:
            raise RuntimeError(f"Failed to acquire Wiki lock after {self.config.concurrency.lock_timeout_seconds}s")
    ...
    async def _commit(self, tx: WikiTransaction) -> int:
        ...
        for op in tx.operations:
            ...
            try:
                if op_type == "create":
                    await self._atomic_write(op["path"], op["content"], frontmatter=op.get("frontmatter", {}))
                    ...
                    count = await self._add_fact_to_page(op["path"], op["content"], op.get("fact"))
                    written_count += count
        ...
        agent_stats = self._stats["by_agent"].setdefault(tx.agent_id, {"transactions": 0, "writes": 0})
        agent_stats["transactions"] += 1
        agent_stats["writes"] += written_count

        if written_count > 0:
            logger.info(f"[{tx.agent_id}] Committed {len(tx.operations)} ops, {written_count} facts written")

        return written_count

    async def _atomic_write(self, rel_path: str, content: str, frontmatter: Dict = None) -> None:
        """
        ...
        if self.config.concurrency.atomic_writes:
            ...
            temp_fd, temp_path = tempfile.mkstemp(dir=str(full_path.parent), suffix=".tmp")
            try:
    ...
    async def _add_fact_to_page(self, rel_path: str, fact_content: str, fact_data: Dict = None) -> int:
        ...
        if full_path.exists():
            ...
            if existing.rstrip().endswith("---"):
                ...
                if last_dash_pos > 0:
                    new_content = existing[:last_dash_pos] + fact_content + "\n---\n"
                else:
    ...
    @staticmethod
    def _is_duplicate_fact(existing_content: str, new_fact_content: str, threshold: float = 0.85) -> bool:
        ...
        new_lines = [
            l.strip()
            for l in new_fact_content.split("\n")
        ...
        for line in new_lines:
            if (
                ...
                and not line.startswith("*Entities*")
        ...
        for block in blocks:
            ...
            for line in block_lines:
                if (
                    ...
                    if ratio >= threshold:
                        logger.debug(f"Duplicate detected (ratio={ratio:.2f}): '{new_core[:50]}' ~ '{line[:50]}'")
                        return True
    ...
    async def _append_changelog(self, log_op: Dict) -> None:
        ...
        entry = (
            f"- **{log_op['timestamp']}** | `{log_op['agent_id']}` | {log_op['change_type']}: {log_op['description']}\n"
        )
    ...
    async def write_facts(self, facts: list, agent_id: str = "default", source: str = "auto_hook") -> int:
        """
