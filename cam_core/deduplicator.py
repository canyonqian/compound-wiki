...
class DedupResult:
    ...
    def total_input(self) -> int:
        return len(self.unique_facts) + len(self.duplicate_facts) + len(self.merged_facts) + len(self.superseded_facts)
...
class ConflictDetector:
    ...
    def check(self, new_fact: "ExtractedFact", existing: "ExistingFact") -> bool:
        ...
        has_negation_in_one = any(n in exist_text for n in negation_words) != any(n in new_text for n in negation_words)
...
class Deduplicator:
    ...
    def __init__(self, config=None, wiki_index=None):
        ...
        self.similarity = SimilarityEngine(method=self.config.deduplication.similarity_method)
    ...
    async def _classify(self, new_fact, existing_facts: list) -> Tuple[MergeAction, Optional[ExistingFact]]:
    ...
    async def _merge_facts(self, new_fact, existing: ExistingFact):
        ...
        merged_confidence = min(0.99, (new_fact.confidence + existing.confidence) / 2 + 0.05)
        ...
        merged = type(new_fact)(
            ...
            tags=merged_tags,
            entities_mentioned=list(set(new_fact.entities_mentioned + existing.__dict__.get("entities_mentioned", []))),
